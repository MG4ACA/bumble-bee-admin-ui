<template>
  <div class="card">
      <TabView>
          <TabPanel header="Brand Table">
            <DataTable v-model:filters="filters" :value="brands" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]" 
              paginatorTemplate="RowsPerPageDropdown FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink"
              currentPageReportTemplate="{first} to {last} of {totalRecords} total brands "
              ...
              <template #header>
                ...
                <span class="p-input-icon-left">
                  ...
                  <InputText v-model="filters['global'].value" placeholder="Search brand" />
                </span>
                  </div>
                  </div>
                
              </template>
              <Column header="Brand Id" field="id"></Column>
              <Column header="Brand Name" field="brandName"></Column>
          ...
          <TabPanel header="Manage Brands">
            ...
            <div class="button-section">
              <Button label="Save Brand" raised />
              <Button label="Delete Brand" raised />
              <Button label="Update Brand" raised />
            </div>
          </TabPanel>
        </TabView>
</div>
...
onMounted(() =>{
  ...
  getBrands(token.value).then(response=>{
    console.log('brnad log here', response);
