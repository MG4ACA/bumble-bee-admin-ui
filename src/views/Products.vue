<template>
  <div class="card">
        <TabView>
            <TabPanel header="Products Table">
              <DataTable v-model:filters="filters" :value="products" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]" 
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords} total products "
              stripedRows sortMode="multiple" tableStyle="min-width: 50rem" filterDisplay="row" :loading="loading">
                ...
                <Column header="Price" field="unitPrice"></Column>
                <Column header="Rating" field="rating"></Column>
                <Column header="Status" field="status"></Column>
                <Column header="Brand-ID" field="brandId"></Column>
                <Column header="Category-Id" field="categoryId"></Column>
            ...
            <TabPanel header="Manage Products">
              <div class="flex mt-3">
              ...
              <div class="flex mt-4">
                ...
                <div class="flex">
                  <InputNumber class="w-11" v-model="value" aria-describedby="username-help" />
                <span class="p-inputgroup-addon">.00</span>
                </div>
              </div>
              <div class="flex flex-column gap-2 col">
                <label for="username">Brand Code</label>
                <DropDown v-model="selectedBrand" :options="brandList" optionLabel="data" placeholder="" class="w-full" />
              </div>
              <div class="flex flex-column gap-2 col">
                <label for="username">Category Code</label>
                <DropDown v-model="selectedCategory" :options="categoryList" optionLabel="data" placeholder="" class="w-full" />
              </div>
              </div>
              <div class="flex mt-4">
                <div class="flex flex-column gap-2 col">
                <label for="username">Ratings</label>
                <InputText id="username" v-model="value" aria-describedby="username-help" class="p-invalid" />
              </div>
...
const brandList =  ref([{data:'test data'}, {data:'test data'}])
const categoryList =  ref([{data:'test data'}, {data:'test data'}])



onMounted(() =>{
  token.value = sessionStorage.getItem("jwt")
  getProducts(token.value).then(response=>{
